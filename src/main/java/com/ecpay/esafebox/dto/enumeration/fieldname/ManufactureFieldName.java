...
@NoArgsConstructor
@AllArgsConstructor
public enum ManufactureFieldName {

	/**
	 * MANUFACTURE
	 */
	MANUFACTURE_ID(EsafeboxFieldName.builder()
	...
	MANUFACTURE_BOXTYPE(EsafeboxFieldName.builder()
			...
			.isMandatory(true)
			.build()),
	MANUFACTURE_QUANTITY(EsafeboxFieldName.builder()
			.fieldName("quantityBox")
			.entityFieldName("quantity")
			.dataType(EsafeboxDataType.LONG)
			.minValue(0L)
			.maxValue(999999999999999999L)
			.isMandatory(true)
	...
	SORTS(EsafeboxFieldName.builder()
			...
			.fixedValues(
				Arrays.asList(
						...
						MANUFACTURE_QUANTITY.getEsafeboxFieldName().getEntityFieldName()
				)
			).build());
